...
from utils.persistance import experiment_name_from_parameters, experiment_exists, load_experiment, save_experiment

def execute_experiment(train_data, test_data, model_class, seed:int, epochs:int, learning_rate:float, 
                       convergence_epsilon=0., growing_epochs_interval=1, saving_epochs_interval=1, 
                       callbacks_epochs_interval=1, callbacks=[], override=False, experiment_name_parameters=None, **experiment):
    experiment.update(dict(seed=seed, epochs=epochs, learning_rate=learning_rate, convergence_epsilon=convergence_epsilon))
    experiment.update(dict(id=experiment_name_from_parameters(experiment_name_parameters, **experiment)))
    ...
    if not override and experiment_exists(**experiment, experiment_name_parameters=experiment_name_parameters):
        ...
        model = model_class.load(**experiment, epoch=len(experiment['train_loss']) - 1).to(device)
    ...
    for callback in callbacks: 
        callback(model=model, train_data=train_data, test_data=test_data, epoch=training_epochs - 1, **experiment)
    ...
    for epoch in range(training_epochs, epochs + 1):
        ...
        if epoch % callbacks_epochs_interval == 0 or epoch == epochs:
            for callback in callbacks: 
                callback(model=model, train_data=train_data, test_data=test_data, epoch=training_epochs - 1, **experiment)
