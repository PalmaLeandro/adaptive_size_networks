...
class TwoLayerNeuralNet(torch.nn.Module, PersistableModel):
    ...
    MODEL_NAME_PARAMETERS = EXPERIMENT_PARAMETERS

    def __init__(self, input_dimension:int, hidden_units:int, bias:bool=True, *args, **kwargs):
        ...
        self.pruning_mask = torch.ones(hidden_units)

        self.store_parameters(input_dimension=input_dimension, hidden_units=hidden_units, bias=bias, **kwargs)
        
    def forward(self, x):
        self.pre_activations = self.input_layer(x) / self.input_layer.weight.norm(dim=1)
        self.activations = self.activation(self.pre_activations) * self.pruning_mask
        dummy_term1 = (
